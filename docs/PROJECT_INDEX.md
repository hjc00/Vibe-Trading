...
| `indicators.py` | 纯 pandas 技术指标函数（2.20）：`compute_macd`（EMA12/26/9，柱=2×(DIF-DEA)）、`compute_kdj`（RSV 平坦→50，K/D 种子 50，1/3 平滑，J=3K-2D）、`compute_bollinger`（中轨 MA、%B=(收-下)/(上-下)、带宽=(上-下)/中）、`find_swing_points`/`find_divergence`（返回 `DivergenceResult`）；被 detectors 与 engine 的逐 bar 序列化共享，确定性可单测 |
...
- **测试**：`tests/stock_tracker/test_{models,signals,engine,risk,store,names,analyzer,track_record,capital_data,valuation_data,sector_data,events_data,events_tools,concept_data,sentiment_data,consensus_data,chip_data,financial_reports_data,concept_tools,indicators}.py` + `tests/api/test_stock_tracker_routes.py`；前端 `__tests__/` 增 `ConceptHeatCard`/`ConsensusCard`/`ChipCard`/`MarketSentimentBar`/`VolumeCard`/`VolumeChartCard` 冒烟测试，`IndicatorChartCard` 四窗格技术指标子图（蜡烛+布林/MA、量、MACD、KDJ，带金叉/死叉与顶/底背离标注）。
