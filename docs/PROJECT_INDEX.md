...
| 文件 | 职责 |
|------|------|
| `models.py` | 配置（`TrackerConfig`/`TrackerThresholds`，阈值支持动态字段；`refresh_interval_seconds` 控制实时行情轮询间隔，`detail_card_count` 控制中间详情卡片数量）+ 快照类型 + 代码归一；`PeriodMetrics` 已含 RPS 分位（`rps_market`/`rps_sector`/`benchmark_return_pct`），`SymbolSnapshot` 已含行业板块（`sector_board`）、风险度量（`risk: RiskMetrics`）与估值质量（`valuation: ValuationSnapshot`）；资金相关模型 `CapitalMetrics`（含 `FundFlowSnapshot`/`MarginSnapshot` 双维度）/`FundFlowHistoryItem`/`MarginHistoryItem` |
...
| `src/components/` | `chat/`(对话/流式/swarm)、`charts/`、`run/`、`portfolio/`、`options/`、`settings/`、`stock-tracker/`、`layout/`、`common/` |
