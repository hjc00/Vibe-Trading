...
- **测试**：`tests/stock_tracker/test_{models,signals,engine,risk,store,names,analyzer,track_record,capital_data,valuation_data,sector_data,events_data,events_tools,concept_data,sentiment_data,consensus_data,chip_data,financial_reports_data,backtest_data,concept_tools,indicators}.py` + `tests/api/test_stock_tracker_routes.py`；前端 `__tests__/` 增 `ConceptHeatCard`/`ConsensusCard`/`ChipCard`/`MarketSentimentBar`/`VolumeCard`/`VolumeChartCard` 冒烟测试，`IndicatorChartCard` 四窗格技术指标子图（蜡烛+布林/MA、量、MACD、KDJ，带金叉/死叉与顶/底背离标注）。
